...
typedef struct {float x; float y; float z; float width; float height; float u; float v;} sprite;
typedef struct {float velocity_x; float velocity_y; size_t t3x_index;} spriteinfo;
...
static spriteinfo spritedata[MAX_SPRITES];
...
static void sceneInit(void)
{
	...
	uLoc_tint = shaderInstanceGetUniformLocation(program.geometryShader, "tint");
	// uLoc_depthinfo = shaderInstanceGetUniformLocation(program.geometryShader, "depthinfo");

	// Configure attributes for use with the vertex shader
	C3D_AttrInfo *vAttrInfo = C3D_GetAttrInfo();
	AttrInfo_Init(vAttrInfo);
	AttrInfo_AddLoader(vAttrInfo, 0, GPU_FLOAT, 3); // v0=position
	AttrInfo_AddLoader(vAttrInfo, 1, GPU_FLOAT, 4); // v1=texture coords
	...
	vbo_data = linearAlloc(MAX_SPRITES * sizeof(sprite));

	for (int i = 0; i < MAX_SPRITES; i++) {
		float width = 400 - SPRITE_WIDTH;
		float height = 240 - SPRITE_HEIGHT;

		spriteinfo si =  {randbetween(-2, 2), randbetween(-2, 2),  randbetween(0, Tex3DS_GetNumSubTextures(t3x_110))};

		const Tex3DS_SubTexture *ts = Tex3DS_GetSubTexture(largetex ? t3x_110 : t3x_64, si.t3x_index);

		sprite s = {randbetween(0, width), randbetween(0, height),
			0.0,
			// MIN_DEPTH + (float)(i + 1) / (float)MAX_SPRITES * DEEPNESS,
	...
	BufInfo_Add(bufInfo, vbo_data, sizeof(sprite), 2, 0x10);
...
static void update(float delta) {
	...
	for (int i = 0; i < current_sprites; i++) {
		spriteinfo *si = &spritedata[i];
		sprite *s = &vbo_data[i];
		s->x += si->velocity_x * delta;
		s->y += si->velocity_y * delta;
...
static void sceneRender(float iod)
{
	...
	C3D_FVUnifSet(GPU_VERTEX_SHADER, uLoc_tint, 1.0f, 0.0f, 1.0f, 1.0f);
	// C3D_FVUnifSet(GPU_VERTEX_SHADER, uLoc_depthinfo, iod, MIN_DEPTH, MAX_DEPTH, DEEPNESS);

	// Draw the VBO
	C3D_DrawArrays(GPU_GEOMETRY_PRIM, 0, current_sprites);
}
